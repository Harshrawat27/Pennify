...
export default function PaywallScreen() {
  ...
  // Load RevenueCat offerings + check trial eligibility
  useEffect(() => {
    getOfferings().then(async (offering) => {
      if (!offering) return;
      let yPkg: PurchasesPackage | null = null;
      ...
      if (customerInfo.originalAppUserId !== userId) {
        Alert.alert(
          ...
          [
            ...
            { text: 'Restore Instead', onPress: () => handleRestore() },
          ]
        );
        return;
      }
      ...
      >
        ...
        <View style={{ height: 88, marginTop: 20, paddingHorizontal: 24 }}>
          {trialEligible ? (
            ...
            // No trial — same title for both plans
            <Text className='text-[30px] font-extrabold text-black text-center leading-9'>
              Subscribe to{'\n'}continue.
            </Text>
          )}
