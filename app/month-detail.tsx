...
export default function MonthDetailScreen() {
  ...
  const stats = useQuery(
    ...
    authenticatedUserId
      ? { userId: authenticatedUserId, startDate: start, endDate: end }
      : 'skip'
  );
            ...
            <View className='flex-row gap-3 mx-4 mb-4'>
              ...
              {trackIncome && (
                <View className='flex-1 bg-white rounded-2xl p-4'>
                  ...
                  <Text
                    className='text-black text-[18px] font-bold mt-0.5'
                    numberOfLines={1}
                  >
            ...
            {filteredTx.length === 0 ? (
              ...
              filteredTx.map((tx) => (
                ...
                >
                    ...
                    <View className='flex-1 ml-3'>
                      ...
                      <View className='flex-row items-center gap-2 mt-1'>
                        <View
                          ...
                          style={{
                            backgroundColor: `${(tx as any).categoryColor ?? '#A3A3A3'}18`,
                          }}
                        >
                          <Text
                            className='text-[10px] font-medium'
                            style={{
                              color: (tx as any).categoryColor ?? '#A3A3A3',
                            }}
                          >
                            {tx.categoryName}
                          </Text>
                        ...
                        <View className='flex-row items-center gap-1'>
                          ...
                          <Text className='text-neutral-400 text-[11px]'>
                            {tx.date.split('-')[2]}{' '}
                            {formatMonthLabel(month).split(' ')[0].slice(0, 3)}
                          </Text>
                        </View>
                      </View>
                    </View>
                  ...
                  {tx.note ? (
                    <Text className='text-neutral-400 text-[12px] mt-2 ml-14'>
                      {tx.note}
                    </Text>
                  ) : null}
                </Pressable>
              ...
              return (
                ...
                >
                  <View className='flex-row items-center gap-3'>
                    <View
                      className={`w-2 h-2 rounded-full ${isSel ? 'bg-black' : 'bg-transparent'}`}
                    />
                    <Text
                      className={`text-[15px] ${isSel ? 'font-bold text-black' : 'font-medium text-neutral-600'}`}
                    >
