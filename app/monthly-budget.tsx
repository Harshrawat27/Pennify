...
export default function MonthlyBudgetScreen() {
  ...
  const prevBudget = useQuery(
    api.monthlyBudgets.getByMonth,
    authenticatedUserId
      ? { userId: authenticatedUserId, month: prevMonth(month) }
      : 'skip'
  );
        ...
        <View className='mx-6'>
          ...
          <View className='bg-neutral-100 rounded-xl px-4 py-3 mb-6 flex-row items-center gap-2'>
            ...
            <Text className='flex-1 text-[12px] text-neutral-500 leading-5'>
              This budget applies from {formatMonthLabel(month)} onwards. It
              will be automatically carried forward every month until you change
              it.
            </Text>
          </View>
          ...
          >
            <Text
              className={`text-[15px] font-bold ${canSave ? 'text-white' : 'text-neutral-400'}`}
            >
